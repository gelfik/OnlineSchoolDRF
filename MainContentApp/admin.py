...
from .models import TeacherAvatar, ExamType, TeacherList, TeacherLink, EducationList
...
class TeacherList_list(admin.ModelAdmin):
    list_display = (
        'subject', 'lastName', 'firstName', 'addDate', 'shortDescription', 'examType', 'avatar', 'is_active',)


@admin.register(EducationList)
class EducationList_list(admin.ModelAdmin):
    list_display = (
        'name', 'addDate', 'shortDescription', 'duration', 'recruitmentStatus', 'is_active',)
